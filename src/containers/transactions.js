...
class Transactions extends Component {
  ...
  renderTransactions() {
    return this.props.allEvents.transactions.map((transaction) => {
      ...
      return (
        <line x1={xPlot} y1="0" x2={xPlot} y2="100%" style={{ stroke: 'rgb(255,0,0)', strokeWidth: 4 }}/>
      );
    });
  }
  ...
  renderTimetext() {
    ...
    for (let xPlot = 0; xPlot < this.props.plotWidth; xPlot += (1000 * this.props.globals.plotScale)) {
      if (xPlot !== 0) {
        secTexts.push(
          <text className="timelineText" x={xPlot - (70 * this.props.globals.plotScale)} y="13" key={xPlot} style={{fill: 'black'}}>
